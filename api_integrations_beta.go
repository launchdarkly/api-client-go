...
Create a Big Segment store integration.
...
You can use either Redis or DynamoDB as your persistent store. When you create a Big Segment store integration, the fields in the `config` object in the request vary depending on which persistent store you use.

If you are using Redis to create your Big Segment store integration, you will need to know:
...
If you are using DynamoDB to create your Big Segment store integration, you will need to know:
...
Delete a Big Segment store integration. Each integration uses either Redis or DynamoDB.
...
func (r ApiValidateConnectionRequest) Execute() (*IntegrationSubscriptionTestEventRep, *http.Response, error) {
...
Validate an integration connection for receiving audit log webhooks.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param integrationKey The integration key
 @param id The integration configuration ID
 @return ApiValidateConnectionRequest
*/
func (a *IntegrationsBetaApiService) ValidateConnection(ctx context.Context, integrationKey string, id string) ApiValidateConnectionRequest {
	return ApiValidateConnectionRequest{
		ApiService: a,
		ctx: ctx,
		integrationKey: integrationKey,
...
// Execute executes the request
//  @return IntegrationSubscriptionTestEventRep
func (a *IntegrationsBetaApiService) ValidateConnectionExecute(r ApiValidateConnectionRequest) (*IntegrationSubscriptionTestEventRep, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodPost
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *IntegrationSubscriptionTestEventRep
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "IntegrationsBetaApiService.ValidateConnection")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/integrations/{integrationKey}/{id}/validate"
	localVarPath = strings.Replace(localVarPath, "{"+"integrationKey"+"}", url.PathEscape(parameterToString(r.integrationKey, "")), -1)
