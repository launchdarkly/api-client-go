...
- `query` is a string that matches against the release pipeline `key`, `name`, and `description`. It is not case sensitive. For example: `?filter=query:examplePipeline`.
	...
	*/
	PatchReleasePipeline(ctx context.Context, projectKey string, pipelineKey string) ApiPatchReleasePipelineRequest

	// PatchReleasePipelineExecute executes the request
	//  @return ReleasePipeline
	PatchReleasePipelineExecute(r ApiPatchReleasePipelineRequest) (*ReleasePipeline, *http.Response, error)
