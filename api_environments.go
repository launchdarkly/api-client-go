...
<blockquote>
    <h3><span>📘</span>Approval settings</h3>
    <p>The <code>approvalSettings</code> key is only returned when the Flag Approvals feature is enabled.</p>
</blockquote>
...
<blockquote>
    <h3><span>📘</span>Approval settings</h3>
    <p>The <code>approvalSettings</code> key is only returned when the Flag Approvals feature is enabled.</p>
    <p>Only the <code>canReviewOwnRequest</code>, <code>canApplyDeclinedChanges</code>, <code>minNumApprovals</code>, <code>required</code> and <code>requiredApprovalTagsfields</code> are editable.</p>
    <p>If you try to patch the environment by setting both <code>required</code> and <code>requiredApprovalTags</code>, it fails and an error appears. Users can specify either required approvals for all flags in an environment or those with specific tags, but not both. Only customers on an Enterprise plan can require approval for flag updates by either mechanism.</p>
</blockquote>
