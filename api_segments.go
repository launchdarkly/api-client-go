...
*/
func (a *SegmentsApiService) DeleteSegment(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiDeleteSegmentRequest {
	return ApiDeleteSegmentRequest{
		ApiService: a,
		ctx: ctx,
...
*/
func (a *SegmentsApiService) GetExpiringUserTargetsForSegment(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiGetExpiringUserTargetsForSegmentRequest {
	return ApiGetExpiringUserTargetsForSegmentRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) GetSegment(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiGetSegmentRequest {
	return ApiGetSegmentRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) GetSegmentMembershipForUser(ctx _context.Context, projectKey string, environmentKey string, segmentKey string, userKey string) ApiGetSegmentMembershipForUserRequest {
	return ApiGetSegmentMembershipForUserRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) GetSegments(ctx _context.Context, projectKey string, environmentKey string) ApiGetSegmentsRequest {
	return ApiGetSegmentsRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) PatchExpiringUserTargetsForSegment(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiPatchExpiringUserTargetsForSegmentRequest {
	return ApiPatchExpiringUserTargetsForSegmentRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
Update a user segment. The request body must be a valid JSON patch, JSON merge patch, or semantic patch.

## Using semantic patches on a segment

To use a [semantic patch](/reference#updates-via-semantic-patches) on a segment resource, you must include a header in the request. If you call a semantic patch resource without this header, you will receive a `400` response because your semantic patch will be interpreted as a JSON patch.
...
1. `instructions` (array): (Required) The list of actions to be performed by the update. Each action in the list must be an object/hash table with a `kind` property that indicates the instruction. Depending on the `kind`, the API may require other parameters. When this is the case, add the parameters as additional fields to the instruction object. Read below for more information on the specific supported semantic patch instructions.

If any instruction in the patch encounters an error, the error will be returned and the segment will not be changed. In general, instructions will silently do nothing if the segment is already in the state requested by the patch instruction. For example, `addIncludedUsers` does nothing when the targets have already been included.
Specific error conditions are noted in the instruction descriptions.

### Instructions

#### `addIncludedUsers`

Adds the user keys in `values` to the individual user targets included in the segment. Returns an error if this causes the same user key to be both included and excluded.
...
Adds the user keys in `values` to the individual user targets excluded from the segment. Returns an error if this causes the same user key to be both included and excluded.
...
Removes the user keys in `values` from the individual user targets included in the segment.
...
Removes the user keys in `values` from the individual user targets excluded from the segment.
...
Updates the name of the segment to the string provided in `value`.
...
*/
func (a *SegmentsApiService) PatchSegment(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiPatchSegmentRequest {
	return ApiPatchSegmentRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) PostSegment(ctx _context.Context, projectKey string, environmentKey string) ApiPostSegmentRequest {
	return ApiPostSegmentRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
...
*/
func (a *SegmentsApiService) UpdateBigSegmentTargets(ctx _context.Context, projectKey string, environmentKey string, segmentKey string) ApiUpdateBigSegmentTargetsRequest {
	return ApiUpdateBigSegmentTargetsRequest{
		ApiService: a,
		ctx: ctx,
		projectKey: projectKey,
