 ...
 * @param projectKey The project key, used to tie the flags together under one project so they can be managed together.
 * @param environmentKey The environment key, used to tie together flag configuration and users under one environment so they can be managed together.
 * @param optional nil or *UserSegmentsApiGetUserSegmentsOpts - Optional Parameters:
...
/*
UserSegmentsApiService Update targets included or excluded in an unbounded segment
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param projectKey The project key, used to tie the flags together under one project so they can be managed together.
 * @param environmentKey The environment key, used to tie together flag configuration and users under one environment so they can be managed together.
 * @param userSegmentKey The user segment&#39;s key. The key identifies the user segment in your code.
...
*/
func (a *UserSegmentsApiService) UpdatedUnboundedSegmentTargets(ctx context.Context, projectKey string, environmentKey string, userSegmentKey string, unboundedSegmentTargetsBody UnboundedSegmentTargetsBody) (*http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Post")
		localVarPostBody   interface{}
		localVarFileName   string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/segments/{projectKey}/{environmentKey}/{userSegmentKey}/unbounded-users"
	localVarPath = strings.Replace(localVarPath, "{"+"projectKey"+"}", fmt.Sprintf("%v", projectKey), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"environmentKey"+"}", fmt.Sprintf("%v", environmentKey), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"userSegmentKey"+"}", fmt.Sprintf("%v", userSegmentKey), -1)
	...
	// body params
	localVarPostBody = &unboundedSegmentTargetsBody
	if ctx != nil {
		// API Key Authentication
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			var key string
