...
type ApiGetApplicationVersionAdoptionRequest struct {
	ctx context.Context
	ApiService *ApplicationsBetaApiService
	applicationKey string
}

func (r ApiGetApplicationVersionAdoptionRequest) Execute() (*ApplicationVersionAdoptionCollection, *http.Response, error) {
	return r.ApiService.GetApplicationVersionAdoptionExecute(r)
}

/*
GetApplicationVersionAdoption Get application version adoption data by application key

Get version adoption data for the last 30 days for a specific application in an account.

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param applicationKey The application key
 @return ApiGetApplicationVersionAdoptionRequest
*/
func (a *ApplicationsBetaApiService) GetApplicationVersionAdoption(ctx context.Context, applicationKey string) ApiGetApplicationVersionAdoptionRequest {
	return ApiGetApplicationVersionAdoptionRequest{
		ApiService: a,
		ctx: ctx,
		applicationKey: applicationKey,
...
// Execute executes the request
//  @return ApplicationVersionAdoptionCollection
func (a *ApplicationsBetaApiService) GetApplicationVersionAdoptionExecute(r ApiGetApplicationVersionAdoptionRequest) (*ApplicationVersionAdoptionCollection, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
		localVarReturnValue  *ApplicationVersionAdoptionCollection
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "ApplicationsBetaApiService.GetApplicationVersionAdoption")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/api/v2/applications/{applicationKey}/version-adoption"
	localVarPath = strings.Replace(localVarPath, "{"+"applicationKey"+"}", url.PathEscape(parameterToString(r.applicationKey, "")), -1)
