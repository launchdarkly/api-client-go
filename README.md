...
For using other server than the one defined on index 0 set context value `ldapi.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `ldapi.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `ldapi.ContextOperationServerIndices` and `ldapi.ContextOperationServerVariables` context maps.
...
auth := context.WithValue(
		context.Background(),
		ldapi.ContextAPIKeys,
		map[string]ldapi.APIKey{
