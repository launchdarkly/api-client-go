...
Search users in LaunchDarkly based on their last active date, or a search query. Do not use to enumerate all users in LaunchDarkly. Instead use the [List users](getUsers) API resource.

<blockquote>
    <h3><span>🚧</span><code>offset</code> is deprecated</h3>
    <p><code>offset</code> is deprecated and will be removed in a future API version. You can still use <code>offset</code> and <code>limit</code> for pagination, but we recommend you use <code>sort</code> and <code>searchAfter</code> instead. <code>searchAfter</code> allows you to page through more than 10,000 users, but <code>offset</code> and <code>limit</code> do not.</p>
</blockquote>
...
<blockquote>
    <h3><span>🚧</span>Pagination has changed</h3>
    <p>This endpoint's pagination mechanism has changed. The <code>next</code> link used to include undocumented <code>h</code> and <code>scrollId</code> parameters. It now uses the new <code>searchAfter</code> parameter. <code>hmac</code> and <code>scrollId</code> parameters are no longer supported.</p>
    <p>Also, users are now sorted by <code>userKey</code>.</p>
</blockquote>
